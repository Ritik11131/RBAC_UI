...
@Component({
  selector: 'app-basic-table-four-example',
...
export class BasicTableFourExampleComponent {
  tableData: ExampleUser[] = [
    {
      id: 1,
      name: 'Lindsey Curtis',
  ...
  columns: TableColumn<ExampleUser>[] = [
    {
      key: 'name',
      label: 'User',
      sortable: true,
    ...
    },
    {
      ...
      sortable: true,
      badgeColor: (item) => {
        if (item.status === 'Hired') return 'success';
        if (item.status === 'In Progress') return 'warning';
        return 'error';
      },
    },
  ];
  ...
  actions: TableAction<ExampleUser>[] = [
    {
      label: 'Edit',
      variant: 'default',
      icon: `<svg width="1em" height="1em" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg" class="size-5">
    ...
    },
    {
      label: 'Delete',
      variant: 'error',
      icon: `<svg width="1em" height="1em" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" class="size-5">
  ...
  onSelectionChange(selected: ExampleUser[]) {
    console.log('Selection changed:', selected);
  }

  onActionClick(event: { action: string; item: ExampleUser }) {
