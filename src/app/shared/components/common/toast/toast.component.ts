...
@Component({
  ...
  template: `
    <div
      ...
      role="alert"
    >
      <h3 [ngClass]="titleClasses" class="font-semibold">
        {{ displayTitle }}
      </h3>
      
      <div [ngClass]="messageClasses" class="mt-2 mb-4">
        {{ toast.message }}
      </div>
...
export class ToastComponent implements OnInit, OnChanges {
  ...
  // Computed properties - updated when toast changes
  toastClasses = '';
  titleClasses = '';
  messageClasses = '';
  actionButtonClasses = '';
  ...
  private readonly toastClassMap: Record<ToastType, string> = {
  ...
  private readonly titleClassMap: Record<ToastType, string> = {
  ...
  private readonly messageClassMap: Record<ToastType, string> = {
  ...
  private readonly actionButtonClassMap: Record<ToastType, string> = {
  ...
  private readonly defaultTitleMap: Record<ToastType, string> = {
    success: 'Success!',
  ...
  private readonly closeIcon = `<svg class="w-3.5 h-3.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
  ...
  private updateComputedProperties(): void {
    const type = this.toast.type || 'info';
    
    this.toastClasses = this.toastClassMap[type] || this.toastClassMap.info;
    this.titleClasses = this.titleClassMap[type] || this.titleClassMap.info;
    this.messageClasses = this.messageClassMap[type] || this.messageClassMap.info;
    this.actionButtonClasses = this.actionButtonClassMap[type] || this.actionButtonClassMap.info;
    this.displayTitle = this.toast.title || this.defaultTitleMap[type] || this.defaultTitleMap.info;
    this.actionIcon = this.closeIcon;
  }
